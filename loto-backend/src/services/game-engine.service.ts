...
export class GameEngineService {
  ...
  generateCard(): LottoCard[] {
    ...
    for (let col = 0; col < 9; col++) {
      ...
      const rowsForColumn = [0, 1, 2].filter((row) =>
        rowFilledColumns[row].includes(col),
      );
  ...
  markNumber(
    ticket: GameTicket,
    drawnNumber: number,
  ): { marked: boolean; markedNumbers: number[] } {
    const markedNumbers = [...ticket.markedNumbers];

    // Check if number exists in card
    const numberExists = ticket.card.some((row) =>
      row.numbers.includes(drawnNumber),
    );
  ...
  isFirstRowCompleted(ticket: GameTicket): boolean {
    ...
    return firstRow.numbers.every((num) =>
      ticket.markedNumbers.includes(num),
    );
  }
  ...
  isAnyRowCompleted(ticket: GameTicket): boolean {
    return ticket.card.some((row) =>
      row.numbers.every((num) => ticket.markedNumbers.includes(num)),
    );
  }
  ...
  distributePrizes(
    totalPool: number,
    winnerCounts: { stage1: number; stage2: number; stage3: number },
  ) {
  ...
  validateTicket(ticket: any): boolean {
    ...
    return ticket.card.every(
      (row: any) =>
        row.row !== undefined &&
        Array.isArray(row.numbers) &&
        row.numbers.length === 5 &&
        row.numbers.every((num: any) => typeof num === 'number' && num >= 1 && num <= 90),
    );
  }
