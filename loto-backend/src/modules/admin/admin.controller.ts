...
import { ApproveDepositDto, RejectDepositDto } from '../../dtos/deposit.dto';
import { ApproveWithdrawDto, RejectWithdrawDto } from '../../dtos/withdraw.dto';
...
export class AdminController {
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Approve deposit from admin panel' })
  async approveDeposit(
    @Body() approveDepositDto: ApproveDepositDto,
    @GetUser('email') adminEmail: string,
  ) {
    return this.adminService.approveDeposit(approveDepositDto.depositId, adminEmail);
  }

  @Post('deposits/reject')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Reject deposit from admin panel' })
  async rejectDeposit(
    @Body() rejectDepositDto: RejectDepositDto,
    @GetUser('email') adminEmail: string,
  ) {
    return this.adminService.rejectDeposit(
      rejectDepositDto.depositId,
      adminEmail,
      rejectDepositDto.reason,
    );
  }
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Approve withdraw from admin panel' })
  async approveWithdraw(
    @Body() approveWithdrawDto: ApproveWithdrawDto,
    @GetUser('email') adminEmail: string,
  ) {
    return this.adminService.approveWithdraw(approveWithdrawDto.withdrawId, adminEmail);
  }

  @Post('withdraws/reject')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Reject withdraw from admin panel' })
  async rejectWithdraw(
    @Body() rejectWithdrawDto: RejectWithdrawDto,
    @GetUser('email') adminEmail: string,
  ) {
    return this.adminService.rejectWithdraw(
      rejectWithdrawDto.withdrawId,
      adminEmail,
      rejectWithdrawDto.reason,
    );
  }
