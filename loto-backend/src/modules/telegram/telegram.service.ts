...
export class TelegramService {
  ...
  async startBot(): Promise<void> {
    ...
    if (webhookUrl) {
      try {
        const secretToken = this.configService.get<string>('TELEGRAM_WEBHOOK_SECRET') ?? undefined;
...
📅 Tarix: ${withdraw.createdAt.toLocaleString()}
  ...
  private async editBotMessage(chatId: string, messageId: string, newText: string): Promise<void> {
    const msgIdStr = messageId.toString();
    const isPhoto = this.photoMessageIds.has(msgIdStr);

    const tryEditText = async () => {
      await this.bot.telegram.editMessageText(chatId, Number(messageId), undefined, newText, {
        parse_mode: 'Markdown',
      });
    };

    const tryEditCaption = async () => {
      await this.bot.telegram.editMessageCaption(chatId, Number(messageId), undefined, newText, {
        parse_mode: 'Markdown',
      });
  ...
  private extractCallbackEntityId(ctx: Context, prefix: string): string | null {
    const telegramCtx = ctx as any;
    const callbackData = telegramCtx.callbackQuery?.data;

    if (typeof callbackData === 'string' && callbackData.startsWith(prefix)) {
      return callbackData.slice(prefix.length).trim() || null;
    }

    if (Array.isArray(telegramCtx.match) && typeof telegramCtx.match[1] === 'string') {
      return telegramCtx.match[1].trim() || null;
    }
  ...
  private async handleApproveDeposit(ctx: Context): Promise<void> {
    ...
    try {
      const depositId = this.extractCallbackEntityId(ctx, 'approve_deposit_');
      if (!depositId) {
        await ctx.answerCbQuery('❌ Depozit ID tapılmadı', { show_alert: true });
        return;
      }
  ...
  private async handleRejectDeposit(ctx: Context): Promise<void> {
    ...
    try {
      const depositId = this.extractCallbackEntityId(ctx, 'reject_deposit_');
      if (!depositId) {
        await ctx.answerCbQuery('❌ Depozit ID tapılmadı', { show_alert: true });
        return;
      }
  ...
  private async handleApproveWithdraw(ctx: Context): Promise<void> {
    ...
    try {
      const withdrawId = this.extractCallbackEntityId(ctx, 'approve_withdraw_');
      if (!withdrawId) {
        await ctx.answerCbQuery('❌ Çıxarış ID tapılmadı', { show_alert: true });
        return;
      }
  ...
  private async handleRejectWithdraw(ctx: Context): Promise<void> {
    ...
    try {
      const withdrawId = this.extractCallbackEntityId(ctx, 'reject_withdraw_');
      if (!withdrawId) {
        await ctx.answerCbQuery('❌ Çıxarış ID tapılmadı', { show_alert: true });
        return;
      }
