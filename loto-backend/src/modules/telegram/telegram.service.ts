...
Created: ${withdraw.createdAt.toLocaleString()}
  ...
  private async handleApproveDeposit(ctx: Context): Promise<void> {
    try {
      const depositId = ctx.match[0].replace('approve_deposit_', '');
      ...
      await this.logBotAction('approval', deposit.userId, deposit.amount, ctx.callbackQuery.id);
    } catch (error) {
  ...
  private async handleRejectDeposit(ctx: Context): Promise<void> {
    try {
      const depositId = ctx.match[0].replace('reject_deposit_', '');
      const deposit = await this.depositModel.findById(depositId);
      ...
      await this.logBotAction('rejection', deposit.userId, deposit.amount, ctx.callbackQuery.id);
    } catch (error) {
  ...
  private async handleApproveWithdraw(ctx: Context): Promise<void> {
    try {
      const withdrawId = ctx.match[0].replace('approve_withdraw_', '');
      ...
      await this.logBotAction('approval', withdraw.userId, withdraw.amount, ctx.callbackQuery.id);
    } catch (error) {
  ...
  private async handleRejectWithdraw(ctx: Context): Promise<void> {
    try {
      const withdrawId = ctx.match[0].replace('reject_withdraw_', '');
      const withdraw = await this.withdrawModel.findById(withdrawId);
      ...
      await this.logBotAction('rejection', withdraw.userId, withdraw.amount, ctx.callbackQuery.id);
    } catch (error) {
