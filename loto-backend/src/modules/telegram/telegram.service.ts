...
export class TelegramService {
  ...
  async startBot(): Promise<void> {
    if (this.bot) {
      // bot.launch() returns a Promise that never resolves (it runs polling indefinitely).
      // We must NOT await it here — doing so blocks NestJS bootstrap and prevents
      // the HTTP server from binding to its port on Render.
...
📅 Tarix: ${deposit.createdAt.toLocaleString()}
    ...
    try {
      ...
      // If screenshot is base64, send as photo
