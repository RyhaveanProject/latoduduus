...
export const AdminAPI = {
  ...
  banUser: (userId: string) => unwrap(api.post('/admin/ban-user', { userId })),
  unbanUser: (userId: string) => unwrap(api.post(`/admin/unban-user/${userId}`)),
  setBalance: (payload: { userId: string; balance: number }) => unwrap(api.post('/admin/set-balance', payload)),
  stats: () => unwrap(api.get('/admin/stats')),
  adminsList: () => unwrap(api.get('/admin/admins/list')),
  ...
  telegramLogs: () => unwrap(api.get('/admin/telegram/logs')),
};
